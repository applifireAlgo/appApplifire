Ext.define('Healthsurvey.healthsurvey.web.com.app.controller.health.SurveyQuestionGroupMainController', {
     extend: 'Healthsurvey.view.fw.frameworkController.FrameworkViewController',
     alias: 'controller.SurveyQuestionGroupMainController',
     ...
     onTreeRefreshClick: function(event, toolEl, panelHeader) {
          ...
          var tree = this.view.down('#SurveyQuestionGroupTree');
     ...
     hideDataBrowser: function() {
          debugger;
          var form = this.view.down('#SurveyQuestionGroup');
          var grid = this.view.down('#SurveyQuestionGroupGrid');
          var tree = this.view.down('#SurveyQuestionGroupTreeContainer');
          grid.setHidden(true);
     ...
     treeClick: function(me, record, item, index, e, eOpts) {
          if (record.data.leaf) {
               var gridPanel = this.view.up().down('#SurveyQuestionGroupGrid');
               ...
               var formPanel = this.getView().up().down("#SurveyQuestionGroup");
     ...
     onGridItemClick: function(me, record, item, index, e, eOpts) {
          var treePanel = this.view.up().up().down('#SurveyQuestionGroupTree');
     ...
     renderFormValue: function(val, me) {
          store = this.view.up().down('#SurveyQuestionGroup').down('#' + me.column.dataIndex + '').store;
     ...
     setGridData: function(data) {
          var gridStore = this.view.down('#SurveyQuestionGroupGrid').store;
          gridStore.removeAll();
     ...
     onDeleteActionColumnClickMainGrid: function(grid, rowIndex) {
          var recordId = grid.store.data.items[rowIndex].data.surveyQuestionGroupId;
          me = this;
     ...
     loadData: function(responseData) {
          this.view.down('#SurveyQuestionGroup').getViewModel().setData(responseData.response.data);
     },
